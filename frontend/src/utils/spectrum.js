 ...
 */
export function visibleSpectrum(result) {
    if (!result)
...
export function nearestBinIndex(frequencies, frequency) {
    let best = 0;
...
export function resolveMagnitude(marker, frequencies, magnitudes) {
    if (frequencies.length === 0)
        return Number.isFinite(marker.magnitude) ? marker.magnitude : null;
